...
- **HTML cards** are embedded as `<iframe sandbox src="profiles/NAME.html" loading="lazy">` in a centered responsive grid (`repeat(auto-fit, minmax(min(100%, 380px), 440px))`, no max-width, so zooming the browser out adds columns for the end-of-class reveal). Each sits in a 480px-tall rounded "stage" with a caption holding two links: the avatar + username go to the student's GitHub profile, and "Open card" opens the card itself. Students can't add external links (card rule), but `build_index.py` is trusted, so the gallery adds the profile link for them — a payoff for naming the file after their real username. The avatar is the username's initial on a per-student hue (a CRC32 of the username, so it's stable across rebuilds) to keep a full grid from looking uniform. Thumbnail iframes get `tabindex="-1"` so each tile is two keyboard stops (profile, card), not three. Until there are 4 cards (`GHOST_TILE_BELOW`), a dashed "Your card goes here" tile is appended.
